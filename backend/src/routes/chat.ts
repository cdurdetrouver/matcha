...
app.get('/:chat_id/:message_id', async (c: Context) => {
	...
	const messages_serialized: MessageType[] = await Promise.all(
		messages.map(async (message: Message) => await message.serialize())
	);
	return c.json({ history: messages_serialized }, 200);
});

app.all('/:chat_id/:message_id', (c: Context) => {
		...
		return {
			onOpen: async (_event, ws) => {
				...
				if (ret_check == null) {
					ws.send(
						JSON.stringify({
							error: 'Server cannot perform checks !',
						})
					);
					return;
				}
				...
				if (message != undefined || user == null) {
					if (message) ws.send(JSON.stringify({ error: message }));
					else
						ws.send(
							JSON.stringify({
								error: 'Server cannot perform checks !',
				...
				if (
					!(await Chats_Users.get_chats_by_user(user.id)).includes(
						chat.id
					)
				) {
			...
			onClose: (_event, ws) => {
				if (chat != null) leaveGroup(chat.id, ws, chan_layer);
			},
