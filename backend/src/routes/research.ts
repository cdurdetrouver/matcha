...
app.post('/all', async (c: Context) => {
	...
	if (tags != undefined && tags.length != 0) {
		...
		user_lists.push(await Tag.get_user_by_tags(tags_list, user.id));
	}
	if (fame_rate != undefined) {
		if (fame_rate < 0 || fame_rate > 1)
			return c.json({ message: 'Fame rate must be between 0 and 1' }, 400);
		user_lists.push(await User.get_all_by_famerate(
			fame_rate, fame_rate === 1 ? 1 : fame_rate + 0.1));
	}
	if (minAge != undefined && maxAge != undefined) {
		if (minAge < 0 || maxAge < 0 || minAge > maxAge)
			return c.json({ message: 'Age must be positive and minAge must be less than maxAge' }, 400);
	...
	const users_list_s = await Promise.all(users_list.map(async (user) => 
