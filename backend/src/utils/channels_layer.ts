...
export function joinGroup(
	...
	chan_layer: Map<number, Set<WSContext<WebSocket>>>
) {
	if (!chan_layer.has(chat_id)) chan_layer.set(chat_id, new Set());
...
export function leaveGroup(
	...
	if (chan_layer.has(chat_id)) {
		...
		if (chan_layer.get(chat_id)!.size === 0) chan_layer.delete(chat_id);
	}
}
