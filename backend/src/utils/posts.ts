...
export async function get_list_users(user: User): Promise<User[]> {
	...
	while(list_user.length < 5 && i < 10) {
		...
		res.map((user: User) => { list_user.push(user); });
		i++;
	}
	return list_user;
}
...
function dist_EarthCoord(lat1: number, lon1: number, lat2: number, lon2: number) {
...
const stopWords = new Set([
	"a", "an", "and", "the", "is", "in", "on", "at", "of", "to", "for", "with", "by", "from", 
	"that", "this", "it", "as", "are", "was", "were", "be", "been", "but", "or", "not", "so"
  ]);

function getImportantWords(text: string): Set<string> {
	return new Set(
	  text.toLowerCase().match(/\b\w+\b/g) // match only words
...
export function grad_users(users_list: User[], user: User): [User, number][] {
	...
	for (let i = 0; i < users_list.length; i++) {
		let note = 0;
		note += (dist_EarthCoord(user.lat, user.long, users_list[i].lat, users_list[i].long) % 100);
		for (let j = 0; j < users_list.length; j++) {
			if (user.interests?.[j] && users_list[i].interests?.includes(user.interests[j])) {
				note+= 5;
			}
		}
		if (users_list[i].wanted == user.wanted)
			note += 25;
		note += count_keyword(user.description ?? "", users_list[i].description ?? "") * 5;
